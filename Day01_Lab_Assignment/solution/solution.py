...
# Bind module to its parent package dynamically to satisfy mock.patch lookup
if __name__ in sys.modules:
    _parts = __name__.split('.')
    if len(_parts) > 1:
        _parent = '.'.join(_parts[:-1])
        if _parent in sys.modules:
            setattr(sys.modules[_parent], _parts[-1], sys.modules[__name__])
...
def compare_models(prompt: str) -> dict:
    ...
    tokens_est = words_count / 0.75
    gpt4o_cost_estimate = (tokens_est / 1000.0) * COST_PER_1K_OUTPUT_TOKENS["gpt-4o"]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        prompt_val = trunc(r.get("prompt", ""))
        gpt4o_resp = trunc(r.get("gpt4o_response", ""))
        mini_resp = trunc(r.get("mini_response", ""))
        gpt4o_lat = f"{r.get('gpt4o_latency', 0.0):.3f}s"
        mini_lat = f"{r.get('mini_latency', 0.0):.3f}s"
        
        lines.append(f"{prompt_val} | {gpt4o_resp} | {mini_resp} | {gpt4o_lat} | {mini_lat}")
